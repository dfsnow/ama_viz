...
```{r state_map, message=FALSE, fig.width=9, fig.height=7, fig.align='center'}
# This chunk does the following:
...
ggplot() +
  ...
  ggsave("plots/p5_doc_movement.pdf", width = 9, height = 7)
...
ggplot() +
  ...
  ggsave("plots/p6_med_schools.pdf", width = 9, height = 7)
...
ggplot() +
  ...
  annotate(
    ...
    color = "grey35",
    lineheight = .9) +
  ggsave("plots/p7_specialists.pdf", width = 9, height = 7)

```

# Genders

```{r gender, message=FALSE, fig.width=9, fig.height=5, fig.align='center'}
gender_years <- c(1960, 1980, 2000, 2010, 2015) 

# Reading in and filtering IPUMS doctor data
gender_data <- read_dta("data/acs_pers_phys.dta") %>%
  mutate(metro = as.numeric(metro)) %>%
  filter(
    (educd >= 114 | year < 2000),              # Edu code changed prior to 2000
    empstat == 1,                              # Must be employed
    age >= 28,                                 # Only docs over 28 are real
    year %in% gender_years                     # Only use years with METRO data
    ) %>%
  mutate(
...
gender_data %>%
  group_by(year, sex, ur) %>%
  summarize(
    income = wtd.quantile(inctot_cpi99, 0.5, weight = perwt) * 1.5,
    age = weighted.mean(uhrswork, perwt)
    ) %>%
  mutate(ur_sex = paste(ur, sex)) %>%
ggplot() +
  geom_line(aes(x = year, y = income, color = ur_sex, linetype = ur_sex)) +
  ...
  scale_color_manual(
    values = c(
      "Rural Female" = ama_colors[length(ama_colors)],
      "Urban Female" = ama_colors[length(ama_colors)],
      "Rural Male" = ama_colors[2],
      "Urban Male" = ama_colors[2]
    )
  ) +
  theme_ama() +
  theme()
...
```{r nurses, message=FALSE, eval=FALSE, fig.width=9, fig.height=8, fig.align='center'}
...
nurses_rank <- left_join(
  ...
  mutate(
    doc_np_rank_diff = abs(doc_count - np_total) > quantile(abs(doc_count - np_total), .85),
    doc_pa_rank_diff = abs(doc_count - pa_total) > quantile(abs(doc_count - pa_total), .85),
    doc_rn_rank_diff = abs(doc_count - rn_total) > quantile(abs(doc_count - rn_total), .85)
  ) %>%
...
nurses_plot1 <- ggplot() +
  geom_line(
    data = nurses_rank %>%
      filter(doc_np_rank_diff == FALSE & type %in% c("doc_count", "np_total")),
    aes(x = type, y = rank, color = factor(doc_np_rank_diff), group = state_code),
    size = 0.9
  ) +
  geom_line(
    data = nurses_rank %>%
      filter(doc_np_rank_diff == TRUE & type %in% c("doc_count", "np_total")),
    aes(x = type, y = rank, color = factor(doc_np_rank_diff), group = state_code),
    size = 0.9
  ) +
  geom_text(
    data = nurses_rank %>% filter(type == "doc_count" & rank %% 2 == 0),
    aes(x = type, y = rank, label = state),
    nudge_x = -0.10, hjust = 1, vjust = 0.5, size = 4.25, color = "grey35"
  ) +
  geom_text(
    data = nurses_rank %>% filter(type == "doc_count" & rank %% 2 == 0),
    aes(x = type, y = rank, label = "─", color = factor(doc_np_rank_diff)),
    nudge_x = 0, hjust = 1, vjust = 0.45, size = 7
  ) +
  geom_text(
    data = nurses_rank %>% filter(type == "doc_count" & rank %% 2 != 0),
    aes(x = type, y = rank, label = state),
    nudge_x = -0.25, hjust = 1, vjust = 0.5, size = 4.25, color = "grey35"
  ) +
  geom_text(
    data = nurses_rank %>% filter(type == "doc_count" & rank %% 2 != 0),
    aes(x = type, y = rank, label = "───", color = factor(doc_np_rank_diff)),
  ...
  geom_text(
    data = nurses_rank %>% filter(type == "np_total" & rank %% 2 == 0),
    aes(x = type, y = rank, label = state),
    nudge_x = 0.10, hjust = 0, vjust = 0.5, size = 4.25, color = "grey35"
  ) +
  geom_text(
    data = nurses_rank %>% filter(type == "np_total" & rank %% 2 == 0),
    aes(x = type, y = rank, label = "─", color = factor(doc_np_rank_diff)),
    nudge_x = 0, hjust = 0, vjust = 0.45, size = 7
  ) +
  geom_text(
    data = nurses_rank %>% filter(type == "np_total" & rank %% 2 != 0),
    aes(x = type, y = rank, label = state),
    nudge_x = 0.25, hjust = 0, vjust = 0.5, size = 4.25, color = "grey35"
  ) +
  geom_text(
    data = nurses_rank %>% filter(type == "np_total" & rank %% 2 != 0),
    aes(x = type, y = rank, label = "───", color = factor(doc_np_rank_diff)),
  ...
  scale_color_manual(values = c(
    "TRUE" = ama_colors[length(ama_colors)],
    "FALSE" = "grey90")
  ) +
  coord_cartesian(clip = "off") +
  labs(
    y = "State Rank (In Doctors/Nurses Per 100k Pop.)",
    x = "Doctors  VS  Nurse Practitioners"
  ) +
  theme_ama() +
  theme(
    legend.position = "none",
    plot.margin = margin(l = 0.5, r = 1.5, t = 1, unit = "lines"),
    panel.border = element_blank(),
    panel.grid = element_blank(),
    axis.title.x = element_text(margin = margin(t = 4)),
    axis.title.y = element_text(margin = margin(r = 35), size = 18),
...
nurses_plot2 <- ggplot() +
  geom_line(
    data = nurses_rank %>%
      filter(doc_rn_rank_diff == FALSE & type %in% c("doc_count", "rn_total")),
    aes(x = type, y = rank, color = factor(doc_rn_rank_diff), group = state_code),
    size = 0.9
  ) +
  geom_line(
    data = nurses_rank %>%
      filter(doc_rn_rank_diff == TRUE & type %in% c("doc_count", "rn_total")),
    aes(x = type, y = rank, color = factor(doc_rn_rank_diff), group = state_code),
    size = 0.9
  ) +
  geom_text(
  ...
  geom_text(
    data = nurses_rank %>% filter(type == "doc_count" & rank %% 2 == 0),
    aes(x = type, y = rank, label = "─", color = factor(doc_rn_rank_diff)),
    nudge_x = 0, hjust = 1, vjust = 0.45, size = 7
  ) +
  geom_text(
    data = nurses_rank %>% filter(type == "doc_count" & rank %% 2 != 0),
    aes(x = type, y = rank, label = state),
    nudge_x = -0.24, hjust = 1, vjust = 0.5, size = 4.25, color = "grey35"
  ) +
  geom_text(
    data = nurses_rank %>% filter(type == "doc_count" & rank %% 2 != 0),
    aes(x = type, y = rank, label = "───", color = factor(doc_rn_rank_diff)),
  ...
  geom_text(
    data = nurses_rank %>% filter(type == "rn_total" & rank %% 2 == 0),
    aes(x = type, y = rank, label = state),
    nudge_x = 0.10, hjust = 0, vjust = 0.5, size = 4.25, color = "grey35"
  ) +
  geom_text(
  ...
  geom_text(
    data = nurses_rank %>% filter(type == "rn_total" & rank %% 2 != 0),
    aes(x = type, y = rank, label = state),
    nudge_x = 0.24, hjust = 0, vjust = 0.5, size = 4.25, color = "grey35"
  ) +
  geom_text(
    data = nurses_rank %>% filter(type == "rn_total" & rank %% 2 != 0),
    aes(x = type, y = rank, label = "───", color = factor(doc_rn_rank_diff)),
  ...
  labs(
    y = "State Rank (In Doctors/Nurses Per 100k Pop.)",
    x = "Doctors  VS  Registered Nurses"
  ) +
  theme_ama() +
  theme(
    legend.position = "none",
    panel.background = element_rect(fill = "transparent"),
    plot.background = element_rect(fill = "transparent"),
    plot.margin = margin(l = 3.3, r = 2, t = 1, unit = "lines"),
    panel.border = element_blank(),
    panel.grid = element_blank(),
    axis.title.x = element_text(margin = margin(t = 4)),
    axis.title.y = element_blank(),
...
nurses_grob_tt <- textGrob(
    "Nurses Serve as Substitutes in Areas with Few Doctors",
    gp = gpar(
      fontsize = 18,
...
nurses_grob_st <- textGrob(
    paste("States with the lowest number of doctors per 100K people",
...
nurses_grob_caption <- textGrob(
    paste0("Sources: American Medical Association Master File",
...
nurses_plot_collection <- grid.arrange(
  arrangeGrob(
    nurses_plot1,
    segmentsGrob(
      x0 = unit(1.5,"npc"),
      ...
      x1 = unit(1.5,"npc"),
      y1 = unit(0.98,"npc"),
      gp = gpar(lwd = 2.0, col = "grey25")
      ),
    nurses_plot2,
    nrow = 1,
    widths = c(1, 0.05, 1),
    clip = "off"),
  top = arrangeGrob(nurses_grob_tt, nurses_grob_st),
  bottom = nurses_grob_caption,
  padding = unit(1.5, "cm")
  
)


ggsave("plots/p8_nurses.png", nurses_plot_collection, width = 9, height = 8)
