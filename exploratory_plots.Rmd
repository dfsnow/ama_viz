...
ggplot(aes(x = factor(year), color = ur, group = ur)) +
  ...
  annotate(
    ...
    y = 240000, yend = 375000, color = "grey45"
  ) +
  annotate(
  ...
  annotate(
    ...
    vjust = 0.245, hjust = 1, color = "grey35"
  ) +
  theme_ama() +
  theme(
    legend.position = "none",
...
# 4. Creates an indicator for the top decile of rank changes
# 5. Creates a rank-change subplot for docs vs nurse practitioners
# 6. Creates a rank-change subplot for docs vs registered nurses
# 7. Combines both plots with grid.arrange and adds title, subtitle, etc
...
nurses_rank <- left_join(
  ...
  mutate(
    doc_np_rank_diff = abs(doc_count - np_total) > quantile(abs(doc_count - np_total), .9),
    doc_pa_rank_diff = abs(doc_count - pa_total) > quantile(abs(doc_count - pa_total), .9),
    doc_rn_rank_diff = abs(doc_count - rn_total) > quantile(abs(doc_count - rn_total), .9)
  ) %>%
  gather(type, rank, np_total:doc_count) 

# Create a plot of state rank, docs per 100k vs NPs per 100k
nurses_plot1 <- ggplot() +
  geom_line(
    data = nurses_rank %>%
  ...
  scale_color_manual(
    ...
    values = c(
      ...
      "FALSE" = "transparent"),
    labels = c(
      "TRUE" = "Top Decile of Rank Changes",
      "FALSE" = ""),
    guide = guide_legend(override.aes = list(
      color = c(NA, ama_colors[length(ama_colors)]),
      size = 8))
  ) +
...
# Create a plot of state rank, docs per 100k vs RNs per 100k
nurses_plot2 <- ggplot() +
  geom_line(
    data = nurses_rank %>%
...
# Create a title grob to arrange with gtable
nurses_grob_tt <- textGrob(
    "Nurses Serve as Substitutes in States with Fewer Doctors",
    gp = gpar(
      fontsize = 18,
...
# Combine all plots and grobs into a single plot
...
# Save final plot
ggsave("plots/p9_nurses.png", nurses_plot_collection, width = 9, height = 8)
