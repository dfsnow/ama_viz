...
- Get census block group data for 2017
- Simplify block group geometries
- Load all AMA data as points
- Spatially intersect AMA points with block groups
- For each block group, calculate number of doctors per person
- Merge this per capita measure back to each point
- Transform and plot point ALPHA according to per capita measure
- Plot point color according to density
...
```{r pop_map, message=FALSE, fig.width=9, fig.height=7, fig.align='center'}
# This chunk does the following:
...
pop_map_bg_geo <- read_rds("data/bg_boundaries_pop.rds") %>%
  ...
  select(-land_area) %>%
  mutate(bg_log_pop = log(bg_population / bg_land_area))
...
pop_map_ama_locs <- pop_map_ama_temp %>%
  ...
  mutate(bg_log_doc = log(bg_doc_pop / bg_land_area))
...
pop_map_breaks <- pop_map_ama_locs %>%
  filter(!is.infinite(bg_log_doc) & !is.na(bg_log_doc)) %>%
  pull(bg_log_doc) %>%
...
pop_map_final <- pop_map_ama_locs %>%
ggplot() +
  geom_sf(
    data = pop_map_states %>%
    filter(!(GEOID %in% c("02", "15"))),
...
# Saving to file
ggsave("plots/p1_pop_map.png", pop_map_final, width = 9, height = 5)
```

```{r pop_map_diff, message=FALSE, fig.width=9, fig.height=7, fig.align='center'}
# This chunk does the following:
# 1. Reads in the block group data created in the prep chunk

pop_diff_ama_locs <- pop_map_ama_locs %>%
  mutate(bg_log_doc = log(bg_doc_pop / bg_land_area)) %>%
  filter(!is.na(bg_log_doc) & !is.infinite(bg_log_doc)) %>%
  mutate(bg_log_pop = log(bg_population / bg_land_area)) %>%
  filter(!is.na(bg_log_pop) & !is.infinite(bg_log_pop))
...
pop_diff_ama_locs <- pop_map_ama_locs %>%
  ...
  filter(!is.na(bg_log_pop) & !is.infinite(bg_log_pop)) %>%
  mutate(
    bg_log_doc_ntile = ntile(bg_log_doc, 20),
    bg_log_pop_ntile = ntile(bg_log_pop, 20),
    bg_diff = bg_log_doc_ntile - bg_log_pop_ntile
    ) %>%
  filter(!is.na(bg_diff) & !is.infinite(bg_diff) & bg_diff > -1)

# Clipping the points from the NY region to place them in a relief
pop_diff_ny_bb_points <- pop_diff_ama_locs %>%
  filter(
    lon >= pop_map_ny_bb$lon[1] & lon <= pop_map_ny_bb$lon[2],
    lat >= pop_map_ny_bb$lat[1] & lat <= pop_map_ny_bb$lat[2]
  ) 
...
# Defines the relief plot to be placed on the main plot
pop_diff_ny_relief_plot <- pop_diff_ny_bb_points %>%
ggplot() +
  geom_sf(
    data = pop_map_states %>%
    filter(!(GEOID %in% c("02", "15"))),
...
# Defining the whole country plot with relief map
pop_diff_final <- pop_diff_ama_locs %>%
ggplot() +
  geom_sf(
    data = pop_map_states %>%
    filter(!(GEOID %in% c("02", "15"))),
  ...
  annotation_custom(
    ggplotGrob(pop_diff_ny_relief_plot),
    xmin = pop_map_ny_relief_bb$lon[1],
    xmax = pop_map_ny_relief_bb$lon[2],
    ymin = pop_map_ny_relief_bb$lat[1],
    ymax = pop_map_ny_relief_bb$lat[2]
...
ggsave("plots/p2_pop_diff.png", pop_diff_final, width = 9, height = 5)
...
```{r load_counties, message=FALSE, eval=FALSE}
...
temp <- counties(states, cb = TRUE, year = 2017)
